...
public class class_Bus_TestingItem
{
    public string id
    {
        set;
...
public class class_Bus_StudentStatusItem
{
    public string symbol
    {
        set;
...
/// <summary>
/// Summary description for class_Bus_Testing
/// </summary>
public class class_Bus_Testing:class_BusBase
{

    class_Data_SqlSPEntry activeSPEntry;
    string _centersymbol = string.Empty;
    Dictionary<string, class_Bus_TestingItem> pool_homeworks = new Dictionary<string, class_Bus_TestingItem>();

    public class_Bus_Testing(class_CommonData refObjectCommonData, string centersymbol) : base(refObjectCommonData)
    {
        _centersymbol = centersymbol;
        activeSPEntry = Object_CommonData.GetActiveSP(Object_CommonData.dbServer, class_SPSMap.SP_OPERATION_CONFIG_TESTING);
    ...
    {
        pool_homeworks = new Dictionary<string, class_Bus_TestingItem>();
        ...
        if (activeDataTable != null && activeDataTable.Rows.Count > 0)
        {
            foreach (DataRow activeDataRow in activeDataTable.Rows)
            {
                ...
                class_Bus_TestingItem newItem = new class_Bus_TestingItem();
    ...
    public XmlDocument GetStatusForAllStudentsWithClass(string classsymbol)
    {
        ...
        if (activeDataTable != null && activeDataTable.Rows.Count > 0)
        {
            ...
            foreach (DataRow activeDataRow in activeDataTable.Rows)
            {
                ...
                class_Data_SqlDataHelper.GetColumnData(activeDataRow, "id", out data_id);
                class_Data_SqlDataHelper.GetColumnData(activeDataRow, "symbol", out data_symbol);
                class_Data_SqlDataHelper.GetArrByteColumnDataToString(activeDataRow, "doc_testing", out data_homework);
                class_Data_SqlDataHelper.GetColumnData(activeDataRow, "owner", out data_owner);
                class_Data_SqlDataHelper.GetArrByteColumnDataToString(activeDataRow, "doc_signed", out data_signed);
                class_Data_SqlDataHelper.GetColumnData(activeDataRow, "date", out data_date);
